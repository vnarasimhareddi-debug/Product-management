...
**Prepared for:** Head of Collections / CTO | **Author:** Solution Architecture, AI/ML — Financial Services (Collections & Debt Recovery)
...
*[Diagram: NBA Engine Component Map — Data Ingestion → Feature Store → {Causal Uplift Model, Constrained Bandit, RL Trainer} → Decision Service → Champion-Challenger Router → Customer Channels, with Explainability Layer wrapping the Decision Service and a Feedback Loop returning outcomes to the Feature Store]*
...
### Data Flow
*[Diagram: Data Flow — Historical Logs (with logging-policy propensities) → OPE Harness → Cleared Policy → Bandit/RL Serving Layer → Feature Store lookup at inference → Customer Interaction → Outcome Logging → back to Historical Logs]*
...
## Page 2 — Implementation Details & Metrics
...
*[Diagram: Deployment Topology — Outbound Batch Path (Airflow → Nightly Scorer → Campaign Queue → Dialer/SMS/WhatsApp Gateway) alongside Inbound Real-Time Path (Agent Console/IVR → FastAPI Decision Service (p99 <200ms) → Redis Feature Cache → Model Registry), both reading/writing a shared Feast Feature Store on Kubernetes]*
